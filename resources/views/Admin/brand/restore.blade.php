...
<html>
  ...
  <body>
    ...
    <div class="x-body">
      <div class="layui-row">
        <form action="{{url('restoreBS')}}" method="get" class="layui-form layui-col-md12 x-so layui-form-pane">
          <input class="layui-input" placeholder="品牌名/品牌ID" name="brand_name">
          <button class="layui-btn">搜索</button>
        </form>
      </div>
      <xblock>
        <a name="recoveryData" class="layui-btn layui-btn-normal"  href="javascript:;">批量恢复</a>
        <a class="layui-btn" href="{{url('brand')}}">返回</a>
        <span class="x-right" style="line-height:40px">本页数据：{{ $allDIsableData->count() }} 条</span>
      </xblock>
      <table class="layui-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
                @if ($v->status == 1) 
                <a name="operateIt" class="layui-btn layui-btn-normal layui-btn-mini" b-id="{{$v->id}}" b-stat="{{$v->status}}" href="#">
                <span>已启用</span>
                </a>
                @else
                <a name="operateIt" style="background:gray;" class="layui-btn layui-btn-normal layui-btn-mini" b-id="{{$v->id}}" b-stat="{{$v->status}}" href="#">
            ...
            <td class="td-manage">
              ...
              <a name="deleteIt" data-id="{{$v->id}}" href="javascript:;">
      ...
      <div class="page">
          {{ $allDIsableData->appends(['searchInfo' => $searchInfo])->links() }}
      </div>
    ...
    <script>
              ...
              $.ajax({
                  ...
                  success:function(data) {
                      ...
                      if (data.code == '666') 
                      {
                        ...
                        layer.msg('数据成功恢复!',{icon:1,time:1000});
      ...
      $("a[name='deleteIt']").click('click',function () {
