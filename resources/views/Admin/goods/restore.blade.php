...
<html>
  ...
  <body>
    <div class="x-nav">
      ...
      <a class="layui-btn layui-btn-small" style="line-height:1.6em;margin-top:3px;float:right" href="{{url('restoreG')}}"  title="刷新">
        <i class="layui-icon" style="line-height:30px">ဂ</i></a>
    </div>
    <div class="x-body">
      <div class="layui-row">
        <form action="{{url('restoreG')}}" method="get" class="layui-form layui-col-md12 x-so layui-form-pane">
          <input class="layui-input" placeholder="商品ID/品名" name="goodsname">
      ...
      <table class="layui-table">
        ...
        @foreach ($goodsInfo as $k => $v)
            ...
            <td bid="{{$v->bid}}">{{$allNeedInfo['brand'][$k]->name}}</td>
            <td tid="{{$v->tid}}">{{$allNeedInfo['type'][$k]->name}}</td>
            <td>{{$v->price}}</td>
            <td>{{$v->store}}</td>
            <td>
                @if ($v->status == 1) 
            ...
            <td class="td-manage">
              ...
              <a name="deleteIt" title="删除" gid="{{$v->id}}" href="{{url('deleteG',[$v->id])}}">
    ...
    <script>
      ...
      $("a[name='recoveryData']").click(function() {

          layer.confirm('确认要恢复数据吗？',function(){
              ...
              $.ajax({
                  type:'get',
                  url:"{{'restoreAllDisabled'}}",
                  data:'id=' + str,
                  success:function(data) {
                    if (data.code == "666") {

                        layer.msg('数据恢复成功!',{icon:1,time:1000});
                        obj.parents('tr').remove();
                        location.href = "{{url('restoreG')}}";
